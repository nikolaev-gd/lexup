...
### Функциональность упрощения текста
- Добавлена кнопка "Simplify" на экране полного текста (FullTextScreen).
- При первом нажатии кнопка отправляет запрос к API ChatGPT для упрощения текста.
- После первого упрощения кнопка переключается между оригинальным и упрощенным текстом без повторных запросов к API.
- Добавлен индикатор загрузки во время обработки запроса к API.
- Реализована обработка ошибок при неудачном запросе к API.
